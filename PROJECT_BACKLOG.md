...
> **Scope change log:** on [this date], scope was deliberately expanded from a single-user research prototype (7 domain entities) to a full account-based platform — registration, login, email verification, password reset, persistent conversation history, and an admin panel (14 domain entities total). This is additive to the Assignment 3 core (still the same screening research), not a pivot, per Assignment 4's Agile-update allowance. Increments 4B, 4C, and revised versions of 6, 7, 9, and 13 below reflect this. REQUIREMENTS.md and DOMAIN_MODEL.md have already been updated; SPECIFICATION.md §1.3 (Individual Scope) still needs a short revision to match — tracked as Increment 1B below.
...
| 3 | Domain Model & Class Diagram *("Assignment 9")* | P3, P5 | DOMAIN_MODEL.md, CLASS_DIAGRAM.md (Mermaid) — now 14 entities (7 research-domain + 7 platform-domain) | §3.2.1 Module/Component Specifications, feeds ERD (Inc. 5) | ✅ Done (expanded) | — |
| 4 | State & Activity Diagrams *("Assignment 8")* | P3, P9 | STATE_DIAGRAMS.md, ACTIVITY_DIAGRAMS.md — now including account lifecycle (registration → verified → active → suspended) and message-persistence workflow | §4.2 Flowchart/Process Flow Diagram (required diagram type #3), §3.2.2 Algorithms and Logic | ⬜ Not started | Medium |
| 4B | **Auth & Account Module Design** *(new — scope expansion)* | P3, P5, P6 | AUTH_DESIGN.md — registration/verification/login/reset flows, session handling approach, admin-panel access boundary | §3.2.1 Module Specs, §8.1.3 Module Decomposition | ✅ Done — **except** FR-018 (profile CRUD), FR-020 (deletion), NFR-014 (erasure), which are simpler flows deferred to a short follow-up or folded into Increment 5 | — |
...
| 5 | Data Model: ERD & AI Data Pipeline *(merges App Dev §8.1.2 + AI/DS §8.3.2)* | P3, P5 | ERD.md/diagram (now covering all 14 entities, including User/Session/Conversation/Message relationships), DATA_PIPELINE.md | §4.4 Data/ERD Diagram (required diagram type #4), §8.1.2, §8.3.2 | ✅ Done — **except** deletion/cascade behavior for account erasure (NFR-014), deferred alongside the same gap already flagged in Increment 4B | — |
| 6 | Repository Layer Design *("Assignment 11")* | P5, P8 | REPOSITORY_DESIGN.md + interface stubs — now including UserRepository, SessionRepository, ConversationRepository, MessageRepository alongside the original research-domain repositories | §3.2.1 Module Specifications, §3.1.3 Tech Stack justification | ⬜ Not started | Medium |
| 7 | Service Layer & REST API *("Assignment 12")* | P5, P9 | SERVICE_LAYER.md, API design, OpenAPI/Swagger stub — now including AuthService (register/login/verify/reset), ConversationService, and AdminService endpoints | §3.2.3 Interface Specifications, §8.4.2 (optional, Web Tech cross-reference), prototype evidence (§5) | ⬜ Not started | Medium |
