...
| **IT Security / Systems Administrator** | Would be responsible for deploying/maintaining a tool like this if adopted | Attack surface of any new tool; whether it introduces new data handling risk itself | Concerned that a "privacy tool" could itself become a new place sensitive data is logged/stored insecurely | Screening logic runs locally/on-prem (no third-party data transmission); minimal new attack surface |
