# MODEL_../architecture/ARCHITECTURE.md — Context-Aware Detector (SentryPrompt4)

**Traces to:** FR-003, FR-004, ../requirements/SPECIFICATION.md §3 (taxonomy), §6 (evaluation approach), NFR-010 (performance budget)
...
**Model:** `llama3.2` (8B, run locally via Ollama)
**Role:** LLM-as-judge classifier — not a fine-tuned or custom-trained model. ../requirements/SPECIFICATION.md §5 explicitly rules out training a model from scratch; this is the documented, permitted alternative.
...
- This is consistent with the feasibility argument already made in ../requirements/SPECIFICATION.md §1.3: the research contribution is the *comparison* between detection strategies, not building a custom model.

---

## 2. Classification Contract

The context-aware detector sends the preprocessed prompt text to `llama3.2` with a fixed, versioned system prompt instructing it to act strictly as a classifier and return structured JSON — not conversational text.
...
No change to ../architecture/ARCHITECTURE.md's Component diagram is required — this fills in what was previously described generically as *"Pretrained NLP model or LLM-as-judge via Ollama"*. The Context-Aware Detector component now specifically means: preprocessed text → fixed prompt template → `llama3.2` inference (temperature 0) → JSON parse → flagged spans passed to the Decision Aggregator, run in parallel with the Rule-Based Detector, exactly as FR-004 requires.
...
| Confidence scores are self-reported by the model, not calibrated | LLM-as-judge confidence values are not statistically calibrated probabilities | Confidence is used for qualitative discussion (../requirements/SPECIFICATION.md §6) only, never as a hard threshold in the aggregation logic |
| Model version drift | `llama3.2` may be updated by Ollama/Meta between now and final submission | Model tag pinned in documentation (not just "llama3.2" but a specific pulled version, to be recorded at evaluation time) |
